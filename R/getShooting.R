...
#' @param DistanceRange Specify how do you want to split the basketball court c("5ft+Range","8ft+Range","By Zone")
#' @param MeasureType Specify if you want data for offensive (base) or defensive(opponent) shooting per player or per team. c("Base","Opponent")
...
#' OpponentTeamID <- c("0")
#'
#' shooting.dataset <- getShooting(Season,Type,DistanceRange,MeasureType,PerMode,SeasonType,SeasonSegment,GameSegment,Period,DateFrom,DateTo,Outcome,OpponentTeamID)
#'
...
url <- gsub("\n","",
paste("http://stats.nba.com/stats/leaguedash",Type,"shotlocations
?College=
&Conference=
&Country=
&DateFrom=",DateFrom,"
...
cols <- unlist(n2$resultSets$headers[4])
ci <- min(which(cols %in% c("TEAM_ID","PLAYER_ID")))


inc <- ifelse(Type == "Team",2,5)

coln <- unlist(lapply(cols[1:(ci-1)], rep, 3))
cols[(ci+inc):length(cols)] <- paste(cols[(ci+inc):length(cols)],coln,sep="_")
colf <-cols[ci:length(cols)]
